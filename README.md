...
```
Signature-origin-classifier/
├── train.py            # Training script: data loading, model, training loop, checkpointing
├── eval.py             # Evaluation script: loads checkpoint, runs batch inference
├── requirements.txt    # Python dependencies
├── README.md           # This file
├── .gitignore          # Excludes venvs, __pycache__, model checkpoints, dataset
│
├── signatures/         # ⚠ Dataset — NOT tracked by Git (see .gitignore)
│   ├── human/          #   790 CSV files — genuine human signatures
│   ├── gan/            #   790 CSV files — GAN-generated signatures
│   ├── sdt/            #   790 CSV files — SDT-generated signatures
│   └── vae/            #   790 CSV files — VAE-generated signatures
│
└── model.pth           # ⚠ Trained checkpoint — NOT tracked by Git (see .gitignore)
                        #   Generated by running train.py on your dataset
```

> **Note on excluded files:** Both `signatures/` and `model.pth` are excluded from version control via `.gitignore`. To use this repository, you must supply your own copy of the dataset in the `signatures/` directory structure shown above, then run `train.py` to produce `model.pth`.
...
| `requirements.txt`| Python dependencies                                                           |
| `.gitignore`      | Excludes virtual environments, caches, model checkpoints, and dataset         |
| `README.md`       | This file                                                                     |
